        ...
        for i in range(self.num_regions):
            for j in range(self.num_regions):
                ...
                if eij > thres or eij2 > thres:
                  ...
                  # add reward
                    if difference_i > 0:
                        cval1[i] = cval1[i] + 0.15*np.abs(cval1[i]) # reward[i]

                    if difference_i < 0:
                        cval1[i] = cval1[i] - 0.10*np.abs(cval1[i]) #punishment[i]

                    if difference_j>0:
                        cval1[j] = cval1[j] + 0.15*np.abs(cval1[j]) #reward[j]
                    if difference_j<0:
                        cval1[j] = cval1[j]  - 0.10*np.abs(cval1[j]) #punishment[j]
