...
class Rice:
    ...
    def __init__(
        ...
        # list of actions and states (1-1 correspondance)
        self.relevant_actions = ['mitigation_rate', 'savings', 'export_action', 'import_actions', 'tariff_actions', 'proposal_actions', 'evaluation_actions']#'green_energy_investment', 'public_transportation_investment']
        self.relevant_states = ['mitigation_rate_all_regions', 'savings_all_regions', 'global_temperature', 'global_carbon_mass']

        self.relevant_action_lens = [1,1, 1, self.num_regions, 2*self.num_regions, self.num_regions, self.num_regions]
        self.relevant_state_lens = [self.num_regions, self.num_regions, 2, 3]
        ...
        self.actions_nvec = (
            ...
            #+ self.green_energy_investment_nvec
            #+ self.public_transportation_investment_nvec
            #+ self.research_development_investment_nvec
        )
    ...
    def phi4(self, actions):
        ...
        for i in range(self.num_regions):
            ...
            for rkey in self.relevant_states:
                ...
                if rlen==1:
                    state_dict[rkey] = rval
                if 1< rlen < self.num_regions:
                    state_dict[rkey] = np.mean(rval)
                if rlen == self.num_regions:
                    state_dict[rkey] = rval[i]"""
            ...
            for akey in self.relevant_actions:
                ...
                if alen == 2*self.num_regions:
                    action_dict[akey] = actions_i[aindex:aindex+self.num_regions]
    ...
    def prepare_grakel_input(self, actions, all_reg_action_dict):
        ...
        grakel_graphs = []
        corr_mat= self.phi3(actions)
        for i in range(self.num_regions):
            #actions_i = actions[i]
            action_attributes_i = all_reg_action_dict[i]
            ...
            for akey in self.relevant_actions:
                ...
                aindex = self.indices_actions[akey]
                alen = self.relevant_action_lens[act_ind]

                # add to the node attributes
                ac1 = action_attributes_i[akey]
            ...
            for i in range(rel_action_len):
                for j in range(rel_action_len):
                    # set edge attr
                    edge_attributes[(i,j)] = corr_mat[i,j]
            ...
            if self.stage == 1:
                ...
                for i in range(self.num_regions):
                    for j in range(self.num_regions):
                        ...
                        # only calculate if the attention coefficient is big enough
    ...
    def reward_punishment(self):
        ...
        for i in range(self.num_regions):
            for j in range(self.num_regions):
                eij = self.kernel_scores[(i,j)]
                eij2 = self.attention_coefficients[i,j]
                if eij > thres: # or eij2 > thres:
                  ...
                  # add reward
                    if difference_i > 0:
                        cval1[i] = cval1[i] + 0.35*np.abs(cval1[i]) # reward[i]

                    if difference_i < 0:
                        cval1[i] = cval1[i] - 0.30*np.abs(cval1[i]) #punishment[i]

                    if difference_j>0:
                        cval1[j] = cval1[j] + 0.35*np.abs(cval1[j]) #reward[j]
                    if difference_j<0:
                        cval1[j] = cval1[j]  - 0.30*np.abs(cval1[j]) #punishment[j]
    ...
    def evaluation_step(self, actions=None):
        ...
        #ac = self.attention_coefficients
        #ac = self.kernel_score_array
        ...
        for i in range(self.num_regions):
            ...
            vec27 = []
            for j in range(self.num_regions):
                coeff_ij = self.kernel_scores[(i,j)] + self.attention_coefficients[i,j] #attention_coefficients[i,j]
